...
// Point 29 (#7): flat list of every identified Error or Compensator across all
// analyses, opened from the Stats Overview cards. Selecting a row opens the item's
// View dialog (ViewError / ViewCompensator), which carries a "View Analysis" button
// to jump to the analysis the item belongs to.
...
import { ErrorIcon, CompensatorIcon, SmallCaretDownIcon } from "@/dialog/components/Icons";
import { colors } from "@/styles/tokens";
import type { ProjectAnalysis, ProjectError, ProjectCompensator } from "@/types/db";

export type StatsItemKind = "errors" | "compensators";

interface Row {
  item: ProjectError | ProjectCompensator;
  analysis: ProjectAnalysis;
}

const META: Record<StatsItemKind, { title: string; subtitle: string; icon: React.ReactNode; primary: string }> = {
  errors: {
  ...
  compensators: {
    ...
    icon: <CompensatorIcon />,
    primary: "View Selected Compensator",
  },
};
...
export default function StatsItemListView({ kind }: { kind: StatsItemKind }) {
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    for (const a of analyses) {
      const items = kind === "errors" ? a.errors ?? [] : a.compensators ?? [];
      for (const item of items) out.push({ item, analysis: a });
    }
    return out;
  }, [initData, kind]);
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        {rows.length === 0 ? (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, color: colors.grey38 }}>
            <span style={{ fontWeight: 700, fontSize: 12.8 }}>No {kind === "errors" ? "errors" : "compensators"} identified yet</span>
      ...
      {viewRow && kind === "errors" && (
        <ViewErrorDialog
          error={viewRow.item as ProjectError}
      ...
      {viewRow && kind === "compensators" && (
        <ViewCompensatorDialog
          compensator={viewRow.item as ProjectCompensator}
          onClose={() => setViewRow(null)}
          onViewAnalysis={goToAnalysis}
        />
      )}
