// src/dialog/components/AnalysisListPortal.tsx
// Full-screen inline portal overlay for viewing/managing analysis list.
...
const COLUMNS: PanelTableCol<ProjectAnalysis>[] = [
  { header: "From Person", width: "27%", render: (a) => a.fromPerson || "—", truncate: true },
  { header: "Analysis Date", width: "24%", render: (a) => a.analysisDate || "—", truncate: true },
  { header: "Analysis Subject", width: "27%", render: (a) => a.analysisSubject || "—", truncate: true },
  { header: "Entity Analyzed", width: "22%", render: (a) => SOURCE_DISPLAY[a.source] ?? a.source ?? "—", truncate: true },
];
...
function EmptyState({ filtered }: { filtered: boolean }) {
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, padding: 32 }}>
      ...
      <span style={{ fontWeight: 700, fontSize: 12.8, color: colors.grey38, lineHeight: "15px" }}>No analysis records found</span>
      <span style={{ fontWeight: 400, fontSize: 11.1, color: colors.grey74, lineHeight: "18px", textAlign: "center", maxWidth: 230 }}>
        {filtered ? "No analyses match the selected filter." : "No analyses have been performed yet."}
      </span>
    </div>
  );
}
...
// ─── Props ──────────────────────────────────────────────────────────────────
interface AnalysisListPortalProps {
      ...
      >
        ...
        <div onMouseDown={onHeaderMouseDown} style={{ height: 78, display: "flex", alignItems: "center", padding: "0 20px", gap: 12, flexShrink: 0, cursor: "grab", userSelect: "none" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontWeight: 400, fontSize: 11.1, lineHeight: "17px", color: colors.grey38, marginTop: 2 }}>
              View and manage all analyses performed.
            </div>
          </div>
        ...
        <div style={{ height: 44, minHeight: 44, background: colors.grey96, display: "flex", alignItems: "center", padding: "0 12px", gap: 8, boxSizing: "border-box" }}>
          <button
            ...
            onClick={() => { if (selectedIndex !== null) { onViewAnalysis(displayRows[selectedIndex]); } }}
          ...
          <button
            title="Provide Feedback With Analysis"
            disabled={!hasSelection}
            onClick={() => { if (hasSelection) setInfoMsg(MSG_PROVIDE); }}
            className="sl-icon-btn"
            style={{ width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", background: "none", border: "none", borderRadius: 4, cursor: hasSelection ? "pointer" : "default", opacity: hasSelection ? 1 : 0.35, flexShrink: 0 }}
          >
            <ProvideFeedbackAnalysisIcon />
          ...
          {/* Flag Analysis as Communication */}
          <button
            title="Flag Analysis as Communication"
            disabled={!hasSelection}
            onClick={() => { if (hasSelection) setInfoMsg(MSG_FLAG); }}
            className="sl-icon-btn"
            style={{ width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", background: "none", border: "none", borderRadius: 4, cursor: hasSelection ? "pointer" : "default", opacity: hasSelection ? 1 : 0.35, flexShrink: 0 }}
          >
            <FlagAnalysisCommunicationIcon />
          ...
          <div ref={filterRef} style={{ position: "relative" }}>
            <button
              ...
              style={{ height: 28, width: 44, display: "flex", alignItems: "center", justifyContent: "center", gap: 3, background: filterSource ? colors.grey92 : "none", border: "none", borderRadius: 4, cursor: "pointer", padding: "0 6px" }}
            ...
            {filterOpen && (
              <div role="menu" style={{ position: "absolute", top: "calc(100% + 2px)", left: 0, zIndex: 200, background: colors.white, border: "1px solid #E0E0E0", boxShadow: "0px 4px 16px rgba(0,0,0,0.12), 0px 1px 4px rgba(0,0,0,0.06)", borderRadius: 4, width: 200 }}>
                {FILTER_OPTIONS.map((opt) => (
                  <button key={opt.label} role="menuitem" onClick={() => handleFilterSelect(opt.value)}
                    style={{ display: "flex", alignItems: "center", gap: 9, width: "100%", textAlign: "left", padding: "9px 15px", background: filterSource === opt.value ? colors.grey95 : "none", border: "none", cursor: "pointer", fontSize: 12.3, color: "#1B1B1B", fontFamily: "Inter, inherit" }}>
                    {opt.icon}
                    {opt.label}
                  </button>
                ))}
          ...
          <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.25)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <div style={{ background: colors.white, border: `1px solid ${colors.grey88}`, borderRadius: 6, padding: "20px 24px", maxWidth: 420, boxShadow: "0px 4px 16px rgba(0,0,0,0.12)" }}>
              <div style={{ fontSize: 12.5, color: colors.grey11, lineHeight: "20px", marginBottom: 18 }}>
                While it is not possible to delete an analysis, however if we want to we can hide it from view. While I cannot delete the selected analysis however I can hide it from view. Do I still want to hide the selected analysis?
              </div>
